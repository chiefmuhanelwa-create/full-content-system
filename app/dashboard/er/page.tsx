...
export default function ErPage() {
  ...
  const tone = (b: string) => TONE[b] ?? '#71717A'
      ...
      <div className="p-6 flex flex-col gap-5 max-w-3xl">
        ...
        <div className="rounded-xl p-4" style={{ background: '#FFF', border: '1px solid #E4E4E7' }}>
          <div className="flex gap-2 flex-wrap">
            ...
            <button onClick={() => run(u)} disabled={busy}
              ...
              style={{ background: '#18181B', color: '#FFF', opacity: busy ? 0.6 : 1 }}>
        ...
        {err && (
          <div className="rounded-xl p-4 flex gap-3" style={{ background: '#FEF2F2', border: '1px solid #FCA5A5' }}>
            ...
            <div>
              <p className="text-sm font-display font-semibold" style={{ color: '#18181B' }}>{err.error}</p>
              {err.how && <p className="text-sm mt-1" style={{ color: '#71717A' }}>{err.how}</p>}
            </div>
          </div>
        )}
        ...
        {d && (
          <>
            <div className="rounded-xl p-6" style={{ background: '#FFF', border: '1px solid #E4E4E7' }}>
              <div className="flex items-center gap-3 mb-5">
                {d.profilePicture && (
                  <img src={d.profilePicture} alt="" className="w-11 h-11 rounded-full object-cover" />
                )}
                <div>
                  <p className="text-base font-display font-bold" style={{ color: '#18181B' }}>@{d.username}</p>
              ...
              <div className="flex items-end gap-3 flex-wrap">
                <div>
                  <p className="text-[10px] font-display font-bold uppercase tracking-widest" style={{ color: '#71717A' }}>
                    Engagement rate
                  </p>
                ...
                <div className="pb-2">
                  ...
                  <p className="text-xs mt-1.5" style={{ color: '#71717A' }}>{d.band.note}</p>
                </div>
              </div>
...
function Cell({ icon: Icon, label, value }: { icon?: any; label: string; value: string }) {
  return (
    <div className="rounded-lg px-3 py-2.5" style={{ background: '#F8F9FA', border: '1px solid #EFEFF1' }}>
      <p className="text-[10px] font-display font-bold uppercase tracking-wide flex items-center gap-1" style={{ color: '#A1A1AA' }}>
        {Icon && <Icon className="w-3 h-3" />}{label}
      </p>
      <p className="text-base font-display font-bold mt-0.5" style={{ color: '#18181B' }}>{value}</p>
    </div>
  )
}

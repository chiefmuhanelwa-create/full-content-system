            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                {CTA_LIBRARY.community.map((cta, i) => (
                  <div key={i} className="flex justify-between items-center p-2 bg-[#F9FAFB] border border-[#E9E5F5] rounded-lg mb-2 text-xs">
                    <span>{cta}</span>
                    <Button variant="ghost" size="sm" onClick={() => copy(cta)}>
                      <Copy className="h-3 w-3" />
                    </Button>
              ...
              <Card>
                ...
                <CardContent>
                  {Object.entries(suggestions.platformSpecific).map(([platform, cta]) => (
                    <div key={platform} className="mb-3 p-3 bg-[#F9FAFB] border border-[#E9E5F5] rounded-lg">
                      <p className="font-display font-bold text-xs text-[#6B6480] mb-1">{platform.toUpperCase()}</p>

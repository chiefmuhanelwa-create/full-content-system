            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                {CTA_LIBRARY.community.map((cta, i) => (
                  <div key={i} className="flex justify-between items-center p-2 bg-purple-50 rounded mb-2 text-xs">
                    <span>{cta}</span>
                    <Button variant="ghost" size="sm" onClick={() => copy(cta)}>
                      <Copy className="h-3 w-3" />
                    </Button>
              ...
              <Card>
                ...
                <CardContent>
                  {Object.entries(suggestions.platformSpecific).map(([platform, cta]) => (
                    <div key={platform} className="mb-3 p-3 bg-gray-50 rounded-lg">
                      <p className="font-semibold text-xs text-gray-600 mb-1">{platform.toUpperCase()}</p>
